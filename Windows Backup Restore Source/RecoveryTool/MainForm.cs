...
namespace BackupRestoreTool
{
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            var target = availableLanguages.FirstOrDefault(x => x.Code == sysLang);
            if (target != null) 
            {
            ...
            {
                // Fallback to Turkish if system lang not found
        ...
        private void InitLanguages()
        {
            ...
            defaultLang["Title"] = "Windows Backup & Recovery Tool";
        ...
        {
            this.Text = "Windows Backup / Restore Tool | made by Abdullah ERTÜRK";
        ...
        private void RunBackupProcess(string sourceRoot, string targetFile)
        {
            ...
            try
            {
                ...
                if (!isWinPE)
                {
                     // VSS Logic
                     string err = "";
                     string driveLetter = sourceRoot.Substring(0, 3); // "C:\" not "C:"
                     
                     string shadowPath = CreateShadowCopyPS(driveLetter, out err);

                     if (!string.IsNullOrEmpty(shadowPath))
                     {
                        LogKey("ShadowPath", shadowPath);
                        
                        // Clean up old mount point if exists (like batch does)
                        if (Directory.Exists(vssMountPath))
                        {
                            try { Directory.Delete(vssMountPath, true); } catch { }
                        }
                        
                        RunProcess("cmd.exe", "/c mklink /D \"" + vssMountPath + "\" \"" + shadowPath + "\\\"");
                        if (Directory.Exists(vssMountPath)) 
                        {
                            captureSource = vssMountPath;
                            vssStatus = 1;
                            LogKey("VSSMounted");
                        }
                     }
                     else
                     {
                        ...
                        // Fallback Logic
                        bool isSystem = sourceRoot.StartsWith("C", StringComparison.OrdinalIgnoreCase);
                        
                        if (Directory.Exists(Path.Combine(sourceRoot, "EFI")))
                        {
                            LogKey("StagingStart");
                            useStaging = true;
                            ...
                            string safeSource = sourceRoot;
                            if (safeSource.EndsWith("\\")) safeSource += "\\";

                            string safeStaging = stagingPath;
                            if (safeStaging.EndsWith("\\")) safeStaging += "\\";

                            string roboArgs = string.Format("\"{0}\" \"{1}\" /E /ZB /R:0 /W:0 /COPY:DAT /XF BCD BCD.LOG /MT:8 /NFL /NDL /NJH /NJS", safeSource, safeStaging);
                            
                            RunProcess("robocopy.exe", roboArgs);

                            if (Directory.Exists(stagingPath))
                            {
                               LogKey("StagingDone");
                               captureSource = stagingPath;
                            }
                            else
                            {
                               LogKey("StagingFail");
                        ...
                        {
                            LogKey("VSSFailSystem");
                            // Don't abort - try direct capture as fallback
                        }
                        else
                        {
                            LogKey("VSSFailNonSystem");
                        }
                     }
                }
                else
                ...
                string compress = "fast"; // default
                if (cbCompress != null)
                {
                    if (cbCompress.SelectedIndex == 0) compress = "none";
                    else if (cbCompress.SelectedIndex == 1) compress = "fast";
                    else if (cbCompress.SelectedIndex == 2) compress = "max";
                }
                
                // ESD always uses recovery compression
                if (targetFile.ToUpper().EndsWith(".ESD")) compress = "recovery";

                if (!captureSource.EndsWith("\\")) captureSource += "\\";
                if (captureSource.EndsWith("\\")) captureSource += "\\";

                // Create scratch directory for DISM (important for network drives)
                string destPath = Path.GetDirectoryName(targetFile);
                string scratchDir = Path.Combine(destPath, "DismTemp");
                if (!Directory.Exists(scratchDir)) Directory.CreateDirectory(scratchDir);

                string tempBat = Path.Combine(Path.GetTempPath(), "run_dism_" + DateTime.Now.Ticks + ".bat");
                
                // ---------------------------------------------------------
                // EXCLUSION LIST LOGIC (v89)
                // ---------------------------------------------------------
                ...
                try
                {
                    ...
                    if (File.Exists(exclusionTemplate))
                    {
                        ...
                        // If [ExclusionList] was missing for some reason, append it? 
                        // Assuming template is valid for now.

                        configFilePath = Path.Combine(Path.GetTempPath(), "dism_config_" + DateTime.Now.Ticks + ".ini");
                        // Use Unicode encoding for DISM compatibility
                        File.WriteAllText(configFilePath, sbEx.ToString(), Encoding.Unicode);
                    }
                }
                catch (Exception ex)
                {
                ...
                string exclusionArg = !string.IsNullOrEmpty(configFilePath) ? " /ConfigFile:\"" + configFilePath + "\"" : "";



                // v95: User requested to DISABLE ESD backup due to performance.
                // Reverting to simple WIM capture logic only.
                string dismCmd = string.Format("dism.exe /Capture-Image /ImageFile:\"{0}\" /CaptureDir:\"{1}\" /Name:\"WindowsBackup\" /Description:\"Created_by_RecoveryTool\" /Compress:{2} /Verify /CheckIntegrity /ScratchDir:\"{3}\"{4}", 
                                               targetFile, captureSource, compress, scratchDir, exclusionArg);
                                               
                // Show command, then message, then run
                string analyzeMsg = GetStr("DismAnalyze");
            ...
            finally
            {
                ...
                if (vssStatus == 1 && !isWinPE) CleanupMount(vssMountPath);
    ...
    if (MessageBox.Show(GetStr("ConfirmRestore"), GetStr("ConfirmTitle"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
        ...
        private void RunRestoreProcess()
        {
            ...
            try
            {
                ...
                string scratchDir = Path.Combine(windowsDrive, "DismTemp");
                if (!Directory.Exists(scratchDir)) Directory.CreateDirectory(scratchDir);

                string dismArgs = string.Format("/Apply-Image /ImageFile:\"{0}\" /Index:{1} /ApplyDir:{2} /ScratchDir:\"{3}\"", wimFile, wimIndex, windowsDrive, scratchDir);
                if (!RunProcess("dism.exe", dismArgs)) 
                { 
                    LogKey("DismApplyError"); 
                    return; 
                }
        ...
        private void KillProcesses()
        {
            try
            {
                // Kill DISM, DiskPart and other related tools if they are still running
                ProcessStartInfo psi = new ProcessStartInfo();
                psi.FileName = "cmd.exe";
                psi.Arguments = "/c taskkill /F /IM dism.exe /T & taskkill /F /IM diskpart.exe /T & taskkill /F /IM format.com /T";
        ...
        private void BtnExit_Click(object sender, EventArgs e)
        {
            ...
            if (isBackupRunning)
            {
                ...
                try
                {
                    ...
                    // 3. Delete Scratch Dir
                    if (!string.IsNullOrEmpty(currentBackupTarget))
                    {
                         string scratchDir = Path.Combine(Path.GetDirectoryName(currentBackupTarget), "DismTemp");
                         if (Directory.Exists(scratchDir))
                         {
                              // Retry loop for cleanup (up to 5 seconds)
                              for (int i = 0; i < 5; i++)
                              {
                                  try 
                                  { 
                                      if (!Directory.Exists(scratchDir)) break;
        ...
        private void GetWimInfo(string wimPath)
        {
             ...
             Thread t = new Thread(() => {
                 try {
                     ...
                     p.StartInfo.Arguments = "/Get-WimInfo /WimFile:\"" + wimPath + "\"";
                     p.StartInfo.UseShellExecute = false;
                     p.StartInfo.RedirectStandardOutput = true;
                     p.StartInfo.CreateNoWindow = true;
                     p.Start();
