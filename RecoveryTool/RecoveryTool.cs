...
{
    /// <summary>
    /// Extracts embedded AppFiles resources to a temp directory at startup
    ...
    public static class EmbeddedResourceExtractor
    {
        ...
        /// <summary>Root temp dir: %TEMP%\RecoveryTool_<exeHash>\</summary>
        ...
        /// <summary>bin\ subfolder inside ExtractedPath</summary>
        /// <summary>bin\ subfolder inside ExtractedPath — same as ExtractedPath since AppFiles are flat</summary>
        public static string BinPath => ExtractedPath;

        /// <summary>Lang\ subfolder inside ExtractedPath</summary>
        public static string LangPath => Path.Combine(ExtractedPath, "Lang");

        public static void Extract()
        {
            // Unique folder per EXE version to avoid stale files
            string exePath = Application.ExecutablePath;
            string hash = Math.Abs(exePath.GetHashCode() ^ File.GetLastWriteTimeUtc(exePath).Ticks.GetHashCode()).ToString("X8");
            _extractedPath = Path.Combine(Path.GetTempPath(), $"RecoveryTool_{hash}");
            ...
            {
                if (!resName.StartsWith("AppFiles", StringComparison.OrdinalIgnoreCase))
                    continue;

                // "AppFiles\wimlib-imagex.exe"  -> "wimlib-imagex.exe"
                // "AppFiles\Lang\tr.ini"        -> "Lang\tr.ini"
                string relativePath = resName.Substring("AppFiles".Length).TrimStart('\\', '/');

                string destFile = Path.Combine(_extractedPath, relativePath);
                string? destDir = Path.GetDirectoryName(destFile);
                if (destDir != null && !Directory.Exists(destDir))
                    Directory.CreateDirectory(destDir);

                // Skip if already extracted (same session)
        ...
        public static void Cleanup()
        {
            ...
            {
                try { Directory.Delete(_extractedPath, true); }
                catch { /* best effort */ }
            }
        }
    }
    ...
    public static class Program
    {
        ...
        static void Main()
        {
            ...
            // Extract embedded AppFiles (bin\, Lang\) to temp
            EmbeddedResourceExtractor.Extract();

            // Point Localization to extracted Lang dir
            Localization.SetLangDir(EmbeddedResourceExtractor.LangPath);

            ImportDigitalSignature();

            Application.EnableVisualStyles();
            ApplicationConfiguration.Initialize();
            Application.ApplicationExit += (_, _) => EmbeddedResourceExtractor.Cleanup();
...
42,8a,36,27,ae,4a,3a,8b,40,f2,ba,db,6f,c9,64,dd,1c,9f";
    ...
    public class PartitionItem
    {
        ...
        public string SizeDisplay => (SizeBytes / 1024.0 / 1024.0 / 1024.0).ToString("F1") + " GB";
