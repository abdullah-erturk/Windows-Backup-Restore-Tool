...
namespace BackupRestoreTool
{
    public partial class MainForm : Form
    {
        ...
        private List<string> _userExclusions = new();
        private static readonly string ExclusionFilePath = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
            "RecoveryTool", "exclusions.txt");
...
42,8a,36,27,ae,4a,3a,8b,40,f2,ba,db,6f,c9,64,dd,1c,9f"";";
        ...
        private void SetupUI()
        {
            ...
            // Exclusion List Editor Button — placed below btnStartBackup, right-aligned
            var btnExclusion = new Button {
                Text = GetStr("UI_BTN_Exclusions") ?? "Exclusion List",
                AutoSize = true,
            ...
            btnExclusion.FlatAppearance.BorderSize = 0;
            btnExclusion.FlatAppearance.MouseOverBackColor = Color.Transparent;
            btnExclusion.Click += (s, e) => ShowExclusionEditor();
            btnExclusion.Tag = "ExclusionBtn";
            tpBackup.Controls.Add(btnExclusion);
            // Position dynamically after layout is done
            tpBackup.Layout += (s, e) => {
                btnExclusion.PerformLayout();
                int x = tpBackup.ClientSize.Width - btnExclusion.Width - 8;
                int y = btnStartBackup.Bottom + 4;
                if (btnExclusion.Location != new Point(x, y))
                    btnExclusion.Location = new Point(x, y);
                btnExclusion.BringToFront();
            };
            LoadUserExclusions();
        ...
        private void tcMain_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (e.Index == 0) tabColor = ColorBackup;
            if (e.Index == 1) tabColor = ColorRestore;
            if (e.Index == 2) tabColor = Color.FromArgb(243, 156, 18);
            if (e.Index == 3) tabColor = ColorNeutral;
            ...
            using (SolidBrush textBrush = new SolidBrush(selected ? Color.Black : Color.White))
            {
                ...
                // Auto-shrink font to fit tab width
                float fontSize = selected ? 10f : 10f;
                FontStyle style = selected ? FontStyle.Bold : FontStyle.Regular;
                using Font baseFont = new Font("Segoe UI", fontSize, style);
                SizeF textSize = g.MeasureString(tp.Text, baseFont);
                if (textSize.Width > rect.Width - 6) {
                    fontSize = Math.Max(7f, fontSize * (rect.Width - 6) / textSize.Width);
                }
                using Font drawFont = new Font("Segoe UI", fontSize, style);
                g.DrawString(tp.Text, drawFont, textBrush, rect, sf);
            }
        ...
        private string GetWimlibPath()
        {
            ...
            // 1. Extracted embedded resource path
            try {
                string embPath = Path.Combine(EmbeddedResourceExtractor.BinPath, exeName);
                if (File.Exists(embPath)) return embPath;
            } catch { }

            // 2. Primary path: app_dir\bin\wimlib-imagex.exe
        ...
        { 
            ...
            Task.Run(() => {
                ...
                try {
                    ...
                    if (_userExclusions.Count > 0) {
                        // Kullanıcı listeyi düzenlemişse onu kullan (varsayılanlar zaten içinde)
                        foreach (string excl in _userExclusions) {
                            string line = excl.Trim();
                            if (!string.IsNullOrEmpty(line))
                                sb.AppendLine(line.StartsWith("\\") ? line : "\\" + line);
                        }
                    } else {
                        // Hiç düzenlenmemişse varsayılan + dinamik listeyi oluştur
                        ...
                        if (Directory.Exists(finalSource + "Users")) {
                            foreach (string userDir in Directory.GetDirectories(finalSource + "Users")) {
                                ...
                                foreach (string cloud in new[] { "OneDrive", "Dropbox", "Google Drive", "SkyDrive" }) {
        ...
        private void LoadUserExclusions()
        {
            _userExclusions.Clear();
            try {
                if (File.Exists(ExclusionFilePath))
                    _userExclusions = File.ReadAllLines(ExclusionFilePath, Encoding.UTF8)
                        .Where(l => !string.IsNullOrWhiteSpace(l)).ToList();
            } catch { }
        }

        private void SaveUserExclusions()
        {
            try {
                string? dir = Path.GetDirectoryName(ExclusionFilePath);
                if (dir != null && !Directory.Exists(dir)) Directory.CreateDirectory(dir);
                File.WriteAllLines(ExclusionFilePath, _userExclusions, Encoding.UTF8);
            } catch { }
        }

        private void ShowExclusionEditor()
        {
            // Varsayılan sistem dışlamaları
            ...
            // Dinamik cloud/temp dışlamaları
            var dynamic = new List<string>();
            string sourceRoot = "";
            if (cmbBackupSource.SelectedItem is PartitionItem pi && !string.IsNullOrEmpty(pi.DrivePath))
                sourceRoot = pi.DrivePath;
            if (!string.IsNullOrEmpty(sourceRoot) && Directory.Exists(sourceRoot + "Users")) {
                foreach (string userDir in Directory.GetDirectories(sourceRoot + "Users")) {
                    string user = Path.GetFileName(userDir);
                    foreach (string cloud in new[] { "OneDrive", "Dropbox", "Google Drive", "SkyDrive" }) {
                        if (Directory.Exists(Path.Combine(userDir, cloud)))
                            dynamic.Add($"\\Users\\{user}\\{cloud}");
                    }
                    if (Directory.Exists(Path.Combine(userDir, "AppData\\Local\\Temp")))
                        dynamic.Add($"\\Users\\{user}\\AppData\\Local\\Temp");
                }
            }

            var defaultFull = defaults.Concat(dynamic)
                .Distinct(StringComparer.OrdinalIgnoreCase).ToList();

            // Eğer kullanıcı daha önce kaydetmemişse varsayılanları göster
            var initialLines = _userExclusions.Count > 0
                ? _userExclusions
                : defaultFull;

            using Form dlg = new Form {
                Text = GetStr("UI_ExclusionTitle") ?? "Exclusion List Editor",
            ...
            Label lblInfo = new Label {
                Text = GetStr("UI_ExclusionInfo") ?? "Paths below will be excluded from backup. One path per line. Example: \\Games",
                Dock = DockStyle.Top,
                Height = 45,
            ...
            TextBox txt = new TextBox {
                ...
                Text = string.Join(Environment.NewLine, initialLines),
                AcceptsReturn = true
            };
            ...
            Button btnReset = new Button {
                Text = GetStr("UI_ExclusionReset") ?? "Load Defaults",
                AutoSize = true,
            ...
            btnReset.Click += (s, e) => {
                txt.Text = string.Join(Environment.NewLine, defaultFull);
            };

            btnPanel.Controls.AddRange(new Control[] { btnSave, btnCancel, btnReset });
        ...
        /// <summary>Set text and auto-shrink font to fit button width.</summary>
        private void SetTextFit(Button? btn, string key, float maxFontSize = 10f, float minFontSize = 7f)
        {
            if (btn == null) return;
            btn.Text = GetStr(key);
            float size = maxFontSize;
            FontStyle style = btn.Font.Style;
            btn.Font = new Font(btn.Font.FontFamily, size, style);
            using Graphics g = btn.CreateGraphics();
            while (size > minFontSize) {
                SizeF sz = g.MeasureString(btn.Text, btn.Font);
                if (sz.Width <= btn.Width - 8) break;
                size -= 0.5f;
                btn.Font = new Font(btn.Font.FontFamily, size, style);
            }
        }
        ...
        {
            // 1. Extracted embedded resource path (highest priority)
            try {
                string embLang = EmbeddedResourceExtractor.LangPath;
                if (Directory.Exists(embLang) && Directory.GetFiles(embLang, "*.ini").Length > 0)
                    return embLang;
            } catch { }

            // 2. Discover Language Folder relative to startup path
            ...
            // 3. Reliability Fallback: Climb up
        ...
        { 
            try {
                ...
                // Auto-shrink header font to fit available width
                if (lblHeader != null) {
                    int availW = lblBootMode.Left - lblHeader.Left - 8;
                    lblHeader.Size = new Size(availW, 40);
                    float hFontSize = 18f;
                    lblHeader.Font = new Font("Segoe UI Semibold", hFontSize, FontStyle.Bold);
                    using Graphics hg = lblHeader.CreateGraphics();
                    while (hFontSize > 10f) {
                        SizeF sz = hg.MeasureString(lblHeader.Text, lblHeader.Font);
                        if (sz.Width <= availW) break;
                        hFontSize -= 0.5f;
                        lblHeader.Font = new Font("Segoe UI Semibold", hFontSize, FontStyle.Bold);
                    }
                }
                ...
                var excBtn = tpBackup.Controls.OfType<Button>().FirstOrDefault(b => b.Tag?.ToString() == "ExclusionBtn");
                if (excBtn != null) excBtn.Text = GetStr("UI_BTN_Exclusions") ?? "Exclusion List";
                SetTextFit(btnDriverBackup, "UI_BTN_DriverBackup", 12f);
                SetTextFit(btnDriverRestore, "UI_BTN_DriverRestore", 12f);
                ...
                // rbWholeDisk — auto-shrink font if text too long for gbStrategy
                rbWholeDisk.Text = GetStr("UI_RB_DiskRestore");
                float rbFontSize = 9f;
                rbWholeDisk.Font = new Font(rbWholeDisk.Font.FontFamily, rbFontSize);
                using (Graphics rg = rbWholeDisk.CreateGraphics()) {
                    int maxW = gbStrategy.Width - rbWholeDisk.Left - 8;
                    while (rbFontSize > 6.5f) {
                        SizeF sz = rg.MeasureString(rbWholeDisk.Text, rbWholeDisk.Font);
                        if (sz.Width <= maxW) break;
                        rbFontSize -= 0.5f;
                        rbWholeDisk.Font = new Font(rbWholeDisk.Font.FontFamily, rbFontSize);
                    }
                }
                ...
                // Auto-shrink BootFix title font
                if (lblBootFixTitle != null) {
                    float bfSize = 16f;
                    lblBootFixTitle.Font = new Font("Segoe UI", bfSize, FontStyle.Bold);
                    using Graphics bg = lblBootFixTitle.CreateGraphics();
                    while (bfSize > 9f) {
                        SizeF sz = bg.MeasureString(lblBootFixTitle.Text, lblBootFixTitle.Font);
                        if (sz.Width <= lblBootFixTitle.Width) break;
                        bfSize -= 0.5f;
                        lblBootFixTitle.Font = new Font("Segoe UI", bfSize, FontStyle.Bold);
                    }
                }
                lblBootFixInfo.Text = (GetStr("UI_LBL_BootFixInfo") ?? "").Replace("\\n", Environment.NewLine);
                SetTextFit(btnAutoBootFix, "UI_BTN_BootFix", 10f);
                SetText(lblBootFixDesc, "UI_LBL_BootFixDesc");
                SetTextFit(btnHealthCheck, "UI_BTN_HealthCheck", 10f);
                ...
                // Resize btnAbout to fit text, then reposition lblBootMode accordingly
                using (Graphics ag = btnAbout.CreateGraphics()) {
                    SizeF asz = ag.MeasureString(btnAbout.Text, btnAbout.Font);
                    int newW = Math.Max(75, (int)asz.Width + 16);
                    int rightEdge = btnAbout.Right;
                    btnAbout.Width = newW;
                    btnAbout.Left = rightEdge - newW;
                    // Keep lblBootMode to the left of btnAbout with 5px gap
                    lblBootMode.Left = btnAbout.Left - lblBootMode.Width - 5;
                    // Recalculate header available width
                    int availW = lblBootMode.Left - lblHeader.Left - 8;
                    lblHeader.Width = availW;
                }
        ...
        private string GetToolPath(string exeName)
        {
            ...
            // 2. Primary path: app_dir\bin\tool.exe
            string binPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "bin", exeName);
            if (File.Exists(binPath)) return binPath;
